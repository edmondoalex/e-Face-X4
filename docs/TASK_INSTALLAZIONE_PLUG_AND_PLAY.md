...
Sesto incremento locale: introdotta la variante di laboratorio `e_face_asterisk/` basata sull'immagine TECH7Fox 6.2.0, con codice provisioner nell'immagine, bootstrap post-init/pre-Asterisk e servizio TLS s6 disabilitato per default. Boot manuale; controllo pre-avvio di porte SIP/HTTP occupate anche se il provisioner è disabilitato; runtime rifiuta include, inventario o endpoint persistenti incoerenti. Test Python e layout passano, **build Docker non ancora eseguita**. Sonda read-only del sito: il volume del PBX attuale è `/mnt/data/supervisor/app_configs/3e533915_asterisk` sul host; la nuova variante avrà un volume diverso per lo slug distinto. Non avviare due PBX e non dichiarare migrati DoorBird/Control4/8301 finché opzioni e file non sono copiati e verificati in una finestra di manutenzione con backup/rollback. Mancano pairing iniziale e test reale di build/riavvio/ricreazione. Nessuna mutazione live.
