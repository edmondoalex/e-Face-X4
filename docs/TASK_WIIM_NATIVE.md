...
La cattura successiva del client WiiM Windows ha mostrato che, durante apertura playlist, avvio e skip, il programma non invia richieste alla porta 49152. I comandi viaggiano verso il WiiM sulla porta 8009 dentro record TLS, mentre il WiiM invia al callback locale del client sulla porta 22334 soltanto eventi UPnP `NOTIFY` in chiaro. Il client desktop non permette quindi di osservare il payload proprietario che genera/rinnova la coda tramite una semplice cattura HTTP. Un gesto effettuato dall'app mobile non attraversa la scheda di rete del PC su una rete commutata: per catturarlo occorre una cattura sul telefono oppure sul punto di accesso/router con mirroring, limitata all'host WiiM e senza persistere token o URL firmati.
...
## Asset player importati nella 2.21.82

Il 15/09/2026 sono state inventariate in sola lettura e importate dal volume persistente `/data/source-icons` le 31 immagini PNG riutilizzabili dei player/sorgenti Control4. Sono escluse deliberatamente configurazione `hidden.json`, cover dei preferiti e cache MDI, perché dati utente o dinamici. Gli asset sono versionati in `app/static/assets/control4-icons/by-id/` e il resolver applica questa precedenza: override persistente dell'utente per ID, asset Git per ID osservato, fallback portabile per nome, cache/lettura Director. In questo modo una futura immagine caricata dall'Admin continua a prevalere e gli asset inclusi non vengono indicati falsamente come personalizzati.

Le associazioni per nome coprono anche installazioni con ID Control4 differenti; le coppie duplicate Deezer `599/1642` e Pandora `593/1654` sono state verificate byte-per-byte identiche. I brani WiiM salvati dalla stella e-Face mostrano inoltre il badge e-Face, mentre i preset provenienti dal WiiM mantengono il badge WiiM.
...
## Protezione preset dinamici 2.21.89

Prova live del 16/09/2026: il preferito Gigi D'Agostino conservava correttamente `tracks/hTQ09bZN0pQ`, preset 1 “Cover e remix” e totale originario 200. Al richiamo, YouTube Music aveva rigenerato sul WiiM una coda omonima di 50 elementi priva di tale ID; il primo elemento era un altro brano. Anche `BrowseQueue` e `BrowseQueueEx` esponevano soltanto quei 50 elementi. Dalla 2.21.89 e-Face mette immediatamente in pausa il preset appena richiamato e avvia `PlayQueueWithIndex` solo dopo aver ritrovato l'ID stabile. Se l'ID non è presente, il preferito resta salvato e nessun brano differente continua a suonare. Senza API del provider o URI persistente non è tecnicamente possibile ricostruire un elemento rimosso dalla coda dinamica.
