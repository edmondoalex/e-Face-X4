...
Sono stati letti, senza eseguirli, i tre `.c4z` forniti. I manifest dichiarano Amazon Music v70, Deezer v148 e Qobuz v23; lo screenshot Composer di Amazon mostra invece Driver Version 79. Le due versioni Amazon espongono almeno la stessa azione `GetLinkForAPIAuthentication` e la stessa proprietà `Authentication URL`, confermate dal file e dallo screenshot reale. Non attribuire alla differenza di versione il fallimento della lettura REST: il test reale sul driver v79 dimostra già che il link non compare nella risposta dell'azione, nella scheda item o nelle variabili. La differenza di versione resta rilevante solo per dettagli interni non verificabili dal pacchetto v70.
...
Esito reale: i tre `GetSettings` e il `LogInCommand` Amazon ricevono un envelope REST `name,result,seq`, ma nessuno restituisce direttamente un link; `OK` nella diagnosi significa solo risposta HTTP, non login riuscito. L'ispezione read-only dell'app Control4 sull'emulatore WSA ha mostrato l'endpoint Director `dataToUi`, già presente nella libreria WebSocket usata da e-Face per gli eventi media. Android riceve inoltre il link come intent esterno verso il browser quando l'utente lo apre. In 2.20.74 una prova admin ha sottoscritto `dataToUi` prima di inviare `LogInCommand` senza osservare un link. In 2.20.75 la prova ha ascoltato prima di inviare l'azione Composer reale `LUA_ACTION / GetLinkForAPIAuthentication`: sul controller reale il link è arrivato negli eventi; tra questi compare anche `UPDATE_PROPERTY`, senza certezza su quale evento contenga il link. In 2.20.76 il flusso è disponibile agli utenti e-Face in Ascolta tramite un endpoint autenticato che restituisce solo il link temporaneo, senza persisterlo. Il test reale del 2026-09-13 ha raggiunto la pagina callback Control4 con `Success!` partendo dal pulsante e-Face, come nell'app Control4. Restano da verificare lo stato account sul controller e la riproduzione.

La procedura ripetibile e le verifiche ancora aperte sono in [TASK_LOGIN_SERVIZI_MUSICALI.md](TASK_LOGIN_SERVIZI_MUSICALI.md). La pista decisiva — ascoltare il flusso eventi prima di inviare il comando — è stata suggerita da Gemini e confermata sul controller reale.
