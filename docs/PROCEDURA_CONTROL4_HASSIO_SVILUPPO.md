...
- Verifica live 2026-09-22: `alexa_devices.send_text_command` non crea in modo affidabile una sveglia dinamica; anche con la frase completa Alexa apre un secondo turno («Una sveglia a che ora?»). Da `2.21.240` usare i servizi companion `eface_alexa.create_alarm`, `create_timer` e `create_reminder`, che riusano in-process la sessione dell'integrazione ufficiale e scrivono `/api/notifications`. Alexa Media Player non è una dipendenza. Dalla `2.21.248` il bootstrap individua la configurazione HA sia in `/homeassistant` sia in `/config`, copia lì `custom_components/eface_alexa`, aggiunge la chiave YAML e controlla `/core/api/services`; se il dominio non è ancora caricato richiede un solo riavvio Core e registra la versione in `/data/.eface_alexa_core_restart`, evitando cicli ai successivi avvii.
- Verifica live `2.21.243`: sull'account europeo la sveglia usa l'endpoint scoperto da `/api/endpoints` e `POST /v1/alerts/alarms`; Amazon risponde `201 Created`, che `aioamazondevices` tratta impropriamente come eccezione. Il companion accetta esplicitamente quella conferma. Provati end-to-end creazione, rilettura e cancellazione della sveglia: `200`, 1 evento attivo, `200`, 0 eventi attivi. Per cancellare usare l'ID restituito da `list_notifications` oppure `delete_next_notification` con tipo `alarm`, `timer` o `reminder`.
