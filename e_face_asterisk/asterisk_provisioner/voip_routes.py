...
_CONTENT = (
    ...
    "exten => _830[2-9],1,Set(EFACE_TRIES=0)\n"
    "same => n(eface_personal_retry),Set(EFACE_TRIES=$[${EFACE_TRIES}+1])\n"
    "same => n,Dial(PJSIP/${EXTEN},40)\n"
    "same => n,GotoIf($[\"${DIALSTATUS}\"=\"ANSWER\" | ${EFACE_TRIES}>=16]?eface_personal_done)\n"
    "same => n,Wait(3)\n"
    "same => n,Goto(eface_personal_retry)\n"
    "same => n(eface_personal_done),Hangup()\n"
    "exten => _83[1-4]X,1,Set(EFACE_TRIES=0)\n"
    "same => n(eface_personal_retry),Set(EFACE_TRIES=$[${EFACE_TRIES}+1])\n"
    "same => n,Dial(PJSIP/${EXTEN},40)\n"
    "same => n,GotoIf($[\"${DIALSTATUS}\"=\"ANSWER\" | ${EFACE_TRIES}>=16]?eface_personal_done)\n"
    "same => n,Wait(3)\n"
