...
const deviceField = (selected, scope = 'all') => `<div class="routine-device-field"><input data-device-search type="search" autocomplete="off" placeholder="Cerca stanza o dispositivo, es. ufficio" aria-label="Cerca dispositivo"><select data-field="device" data-device-scope="${scope}">${deviceOptions(selected, scope === 'safe' ? safeDevices() : scope === 'media' ? devices.filter(item => item.kind === 'media_player') : scope === 'alexa_schedule' ? devices.filter(item => item.kind === 'alexa_schedule') : scope === 'cover_light' ? devices.filter(item => ['cover','light'].includes(item.kind)) : devices)}</select></div>`
const alexaScheduleFields = item => `${deviceField(item.device_id,'alexa_schedule')}<label>Anticipo / ritardo (minuti)<input data-field="alexa-offset" type="number" min="-180" max="180" step="1" value="${Number(item.offset_minutes)||0}"></label>`
