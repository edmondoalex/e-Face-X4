...
$('#security-camera-add').addEventListener('click',()=>{securityCameras.push({id:(crypto.randomUUID?.()||`${Date.now()}-${Math.random()}`).replaceAll('-','').slice(0,32),name:'',url:''});renderSecurityCameras();markSecurityDirty();$('#security-camera-list input')?.focus()})
...
async function saveSecurityCameras(message='Salvato ✓'){const response=await fetch(apiUrl('../api/user/appearance'),{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({security_order:orderedSecurity,security_cameras:securityCameras.map(camera=>({...camera,name:camera.name.trim(),url:camera.url.trim()}))})});const data=await response.json();if(!response.ok)throw new Error(data.detail);securityCameras=data.security_cameras||securityCameras;renderSecurityCameras();securityStatus(message,'saved')}
