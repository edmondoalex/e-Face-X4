...
async function loadPlayers() {
  ...
  $('#skyq-services').innerHTML = detectedApps.map(app => `<label data-skyq-app="${esc(app.title)}" class="${selectedApps.has(String(app.title).toLocaleLowerCase()) ? 'selected' : ''}"><button type="button" class="skyq-app-drag" aria-label="Trascina ${esc(app.title)}" title="Trascina per riordinare">☰</button><span class="skyq-app-icon"><i class="mdi-mask" style="--icon:url('${apiUrl(`../api/icons/mdi/${fallbackIcon(app.title)}.svg`)}')"></i><img src="${apiUrl(`../api/skyq/apps/${encodeURIComponent(app.id)}/icon`)}" alt="" loading="lazy" onload="this.parentElement.classList.add('loaded')" onerror="this.hidden=true"></span><strong>${esc(app.title)}</strong><input type="checkbox" value="${esc(app.title)}" ${selectedApps.has(String(app.title).toLocaleLowerCase()) ? 'checked' : ''}></label>`).join('') || '<p class="skyq-apps-empty">Nessuna app letta. Verifica che Sky Q sia acceso, poi premi SALVA E VERIFICA DIRETTA.</p>'
...
function ensureSkyqPanel() {
  ...
  document.head.insertAdjacentHTML('beforeend', '<style>.skyq-form{margin:18px 0}.skyq-apps-head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-top:18px}.skyq-apps-actions{display:flex;gap:8px}.skyq-apps-actions button{padding:7px 11px;font-size:12px}.skyq-services{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:10px;margin-top:10px}.skyq-services label{position:relative;display:grid;grid-template-columns:24px 48px minmax(0,1fr);align-items:center;gap:8px;min-height:64px;padding:8px 11px;border:1px solid var(--line);border-radius:12px;background:rgba(255,255,255,.055);cursor:pointer;transition:.15s ease}.skyq-services label:active{transform:scale(.97)}.skyq-services label.dragging{z-index:2;opacity:.58;border-color:var(--cyan);transform:scale(.97)}.skyq-services label.selected{border-color:var(--cyan);background:rgba(75,210,240,.12);box-shadow:0 0 16px rgba(75,210,240,.12)}.skyq-app-drag{width:24px;min-width:24px;height:42px;padding:0;border:0;background:transparent;color:#9ba8ab;cursor:grab;touch-action:none}.skyq-app-drag:active{cursor:grabbing}.skyq-app-icon{position:relative;display:block;width:48px;height:48px;padding:0!important;border-radius:9px;background:#181b1d}.skyq-app-icon i,.skyq-app-icon img{position:absolute;inset:5px;width:38px;height:38px}.skyq-app-icon i{background:#dce6e8}.skyq-app-icon img{object-fit:contain}.skyq-app-icon.loaded i{display:none}.skyq-services strong{padding-right:15px;overflow:hidden}.skyq-services input{position:absolute;right:8px;top:8px;width:17px;height:17px;accent-color:var(--cyan)}.skyq-apps-empty{grid-column:1/-1;color:var(--muted)}</style>')
