...
const homeWidgetLabels={overview:['Stato casa','Sicurezza e comfort'],states:['Contatori rapidi','Luci, extra, oscuranti e scorciatoie'],rooms:['Ambienti','Accesso rapido alle stanze'],live:['Sessioni live','Audio e video in riproduzione']}
...
homeWidgetCard.addEventListener('click',async()=>{try{const response=await fetch(apiUrl('../api/user/appearance'),{cache:'no-store'});if(!response.ok)throw new Error(`HTTP ${response.status}`);homeWidgets=(await response.json()).home_widgets||[];renderHomeWidgetAdmin();homeWidgetPanel.hidden=false}catch(error){notice(error.message)}})
