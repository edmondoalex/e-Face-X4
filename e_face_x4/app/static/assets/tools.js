...
const homeWidgetPanel=document.createElement('section');homeWidgetPanel.className='media-config';homeWidgetPanel.hidden=true;homeWidgetPanel.innerHTML='<header><button type="button" data-home-widget-back>‹</button><div><h2>Home dinamica</h2></div></header><p>Trascina i widget nell’ordine desiderato, scegli la dimensione e nascondi ciò che non ti serve.</p><label>Entità ultimo evento videosorveglianza<input id="home-camera-entity" placeholder="camera.nvr_32ch_ext_ultimo_evento"></label><div id="home-widget-list" class="shortcuts-order-list"></div><footer><button type="button" id="home-widget-save" class="user-save">SALVA HOME</button></footer>';document.querySelector('.tools-shell').append(homeWidgetPanel)
let homeWidgets=[]
function renderHomeWidgetAdmin(){ $('#home-widget-list').innerHTML=homeWidgets.map((item)=>{const label=homeWidgetLabels[item.id]||[item.id,''];return `<div class="shortcut-admin-device home-widget-admin" data-home-widget-admin="${item.id}"><button type="button" class="shortcut-drag-handle" data-home-widget-drag aria-label="Trascina ${label[0]}">☰</button><span><b>${label[0]}</b><small>${label[1]}</small></span><select aria-label="Dimensione ${label[0]}"><option value="compact" ${item.size==='compact'?'selected':''}>Compatto</option><option value="standard" ${item.size==='standard'?'selected':''}>Medio</option><option value="wide" ${item.size==='wide'?'selected':''}>Largo</option></select><input type="checkbox" aria-label="Mostra ${label[0]}" ${item.visible?'checked':''}></div>`}).join('') }
function syncHomeWidgets(){homeWidgets=[...$('#home-widget-list').querySelectorAll('[data-home-widget-admin]')].map((row)=>({id:row.dataset.homeWidgetAdmin,visible:row.querySelector('input').checked,size:row.querySelector('select').value}))}
homeWidgetCard.addEventListener('click',async()=>{try{const response=await fetch(apiUrl('../api/user/appearance'),{cache:'no-store'});if(!response.ok)throw new Error(`HTTP ${response.status}`);const appearance=await response.json();homeWidgets=appearance.home_widgets||[];$('#home-camera-entity').value=appearance.home_camera_entity||'camera.nvr_32ch_ext_ultimo_evento';renderHomeWidgetAdmin();homeWidgetPanel.hidden=false}catch(error){notice(error.message)}})
...
$('#home-widget-save').addEventListener('click',async()=>{try{syncHomeWidgets();const response=await fetch(apiUrl('../api/user/appearance'),{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({home_widgets:homeWidgets,home_camera_entity:$('#home-camera-entity').value.trim()})});if(!response.ok)throw new Error((await response.json()).detail);notice('Home dinamica salvata');homeWidgetPanel.hidden=true}catch(error){notice(error.message)}})
