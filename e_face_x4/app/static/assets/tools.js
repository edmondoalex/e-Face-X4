...
const securityLabels = {scenarios:'SCENARI DI INSERIMENTO', areas:'STATO AREE', zones:'ZONE', locks:'SERRATURE', cameras:'VIDEOCAMERE'}
...
function renderSecurityCameras() {
  $('#security-camera-list').innerHTML = securityCameras.map((camera,index)=>`<div class="security-camera-admin" data-camera-index="${index}"><button type="button" class="drag-handle" data-camera-drag aria-label="Trascina ${esc(camera.name||'videocamera')}">☰</button><label>Nome<input data-camera-name maxlength="100" value="${esc(camera.name)}" placeholder="Es. Ingresso"></label><label>Link o entità e-Control<input data-camera-url maxlength="2000" value="${esc(camera.url)}" placeholder="camera.ingresso oppure https://…"></label><button type="button" data-camera-remove aria-label="Elimina videocamera">×</button></div>`).join('') || '<p>Nessuna videocamera configurata.</p>'
}
$('#security-order-tool').addEventListener('click', async () => { try { const response = await fetch(apiUrl('../api/user/appearance'), {cache:'no-store'}); if (!response.ok) throw new Error(`HTTP ${response.status}`); const data=await response.json();orderedSecurity=data.security_order||Object.keys(securityLabels);securityCameras=data.security_cameras||[];renderSecurityOrder();renderSecurityCameras();$('#security-order-config').hidden = false } catch (error) { notice(error.message) } })
...
$('#security-camera-add').addEventListener('click',()=>{securityCameras.push({id:(crypto.randomUUID?.()||`${Date.now()}-${Math.random()}`).replaceAll('-','').slice(0,32),name:'',url:''});renderSecurityCameras();$('#security-camera-list input')?.focus()})
$('#security-camera-list').addEventListener('input',event=>{const row=event.target.closest('[data-camera-index]');if(!row)return;const camera=securityCameras[Number(row.dataset.cameraIndex)];if(event.target.matches('[data-camera-name]'))camera.name=event.target.value;if(event.target.matches('[data-camera-url]'))camera.url=event.target.value})
$('#security-camera-list').addEventListener('click',event=>{const button=event.target.closest('[data-camera-remove]');if(!button)return;securityCameras.splice(Number(button.closest('[data-camera-index]').dataset.cameraIndex),1);renderSecurityCameras()})
...
const finishCameraDrag=()=>{if(!draggedCamera)return;const previous=[...securityCameras];securityCameras=[...$('#security-camera-list').querySelectorAll('[data-camera-index]')].map(row=>previous[Number(row.dataset.cameraIndex)]);draggedCamera.classList.remove('dragging');draggedCamera=null;renderSecurityCameras()}
...
$('#security-order-save').addEventListener('click', async () => { try { const response = await fetch(apiUrl('../api/user/appearance'), {method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify({security_order:orderedSecurity,security_cameras:securityCameras.map(camera=>({...camera,name:camera.name.trim(),url:camera.url.trim()}))})}); if (!response.ok) throw new Error((await response.json()).detail); notice('Sicurezza e videocamere salvate'); $('#security-order-config').hidden = true } catch (error) { notice(error.message) } })
...
const homeWidgetLabels={overview:['Stato casa','Sicurezza e comfort'],weather:['Meteo','Condizioni rilevate da Home Assistant'],camera_event:['Videosorveglianza','Ultimo evento NVR esterno'],doorbell:['DoorBird · chiamata','Ultima immagine campanello'],motion:['DoorBird · movimento','Ultimo evento del sensore'],states:['Contatori rapidi','Luci, extra, oscuranti e scorciatoie'],rooms:['Ambienti','Accesso rapido alle stanze'],live:['Sessioni live','Audio e video in riproduzione']}
...
const homeWidgetPanel=document.createElement('section');homeWidgetPanel.className='media-config';homeWidgetPanel.hidden=true;homeWidgetPanel.innerHTML='<header><button type="button" data-home-widget-back>‹</button><div><h2>Home dinamica</h2></div></header><p>Componi la Home su una griglia a 12 colonne: trascina, dimensiona e guarda subito il risultato nell’anteprima.</p><label>Località meteo<input id="home-weather-location" maxlength="100" placeholder="Es. Torino, Piemonte"></label><label>Entità ultimo evento videosorveglianza<input id="home-camera-entity" placeholder="camera.nvr_32ch_ext_ultimo_evento"></label><div id="home-widget-preview" class="home-widget-preview" aria-label="Anteprima disposizione Home"></div><div id="home-widget-list" class="shortcuts-order-list"></div><footer><button type="button" id="home-widget-save" class="user-save">SALVA HOME</button></footer>';document.querySelector('.tools-shell').append(homeWidgetPanel)
document.head.insertAdjacentHTML('beforeend','<style>.home-widget-preview{display:grid;grid-template-columns:repeat(12,minmax(0,1fr));grid-auto-flow:dense;gap:7px;margin:16px 0;padding:12px;border-radius:16px;background:#0d1518;box-shadow:inset 0 0 0 1px #ffffff12}.home-widget-preview>div{grid-column:span var(--widget-span);min-width:0;min-height:64px;padding:10px;display:flex;flex-direction:column;justify-content:space-between;border-radius:10px;color:#eff7f6;background:linear-gradient(135deg,#17444a,#20282c);box-shadow:inset 0 0 0 1px #64e8bd33}.home-widget-preview span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-weight:700}.home-widget-preview small{color:#68e4bb}.home-widget-admin{grid-template-columns:auto minmax(150px,1fr) minmax(74px,auto) minmax(74px,auto) auto!important}.home-widget-admin select{min-height:38px}@media(max-width:700px){.home-widget-preview{grid-template-columns:repeat(6,minmax(0,1fr))}.home-widget-preview>div{grid-column:1/-1!important}.home-widget-admin{grid-template-columns:auto minmax(0,1fr) auto!important}.home-widget-admin select{grid-column:2}.home-widget-admin input{grid-column:3;grid-row:1}}</style>')
let homeWidgets=[]
function renderHomeWidgetPreview(){const spans={quarter:3,compact:4,standard:6,large:8,wide:12};$('#home-widget-preview').innerHTML=homeWidgets.filter((item)=>item.visible).map((item)=>`<div style="--widget-span:${spans[item.size]||6}" data-preview-widget="${item.id}"><span>${homeWidgetLabels[item.id]?.[0]||item.id}</span><small>${spans[item.size]||6}/12</small></div>`).join('')||'<p>Nessun widget visibile</p>'}
function renderHomeWidgetAdmin(){ $('#home-widget-list').innerHTML=homeWidgets.map((item)=>{const label=homeWidgetLabels[item.id]||[item.id,''],image=['camera_event','doorbell','motion'].includes(item.id),height=image?`<select data-widget-height aria-label="Altezza ${label[0]}"><option value="uniform" ${item.height==='uniform'?'selected':''}>Uniforme alla riga</option><option value="short" ${item.height==='short'?'selected':''}>Bassa</option><option value="standard" ${!item.height||item.height==='standard'?'selected':''}>Media</option><option value="tall" ${item.height==='tall'?'selected':''}>Alta</option></select>`:'';return `<div class="shortcut-admin-device home-widget-admin" data-home-widget-admin="${item.id}"><button type="button" class="shortcut-drag-handle" data-home-widget-drag aria-label="Trascina ${label[0]}">☰</button><span><b>${label[0]}</b><small>${label[1]}</small></span><select data-widget-size aria-label="Larghezza ${label[0]}"><option value="quarter" ${item.size==='quarter'?'selected':''}>25%</option><option value="compact" ${item.size==='compact'?'selected':''}>33%</option><option value="standard" ${item.size==='standard'?'selected':''}>50%</option><option value="large" ${item.size==='large'?'selected':''}>66%</option><option value="wide" ${item.size==='wide'?'selected':''}>100%</option></select>${height}<input type="checkbox" aria-label="Mostra ${label[0]}" ${item.visible?'checked':''}></div>`}).join('');renderHomeWidgetPreview() }
...
$('#home-widget-save').addEventListener('click',async()=>{try{syncHomeWidgets();const response=await fetch(apiUrl('../api/user/appearance'),appearanceOptions({method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({home_widgets:homeWidgets,home_camera_entity:$('#home-camera-entity').value.trim(),home_weather_location:$('#home-weather-location').value.trim()})}));if(!response.ok)throw new Error((await response.json()).detail);notice('Home salvata · meteo e telecamera aggiornati su tutte le UI');homeWidgetPanel.hidden=true}catch(error){notice(error.message)}})
