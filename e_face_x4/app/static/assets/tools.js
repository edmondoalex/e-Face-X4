...
function renderSecurityCameras() {
  $('#security-camera-list').innerHTML = securityCameras.map((camera,index)=>`<div class="security-camera-admin" data-camera-index="${index}"><button type="button" class="drag-handle" data-camera-drag aria-label="Trascina ${esc(camera.name||'videocamera')}">☰</button><label>Nome<input data-camera-name maxlength="100" value="${esc(camera.name)}" placeholder="Es. Ingresso"></label><label>Link<input data-camera-url maxlength="2000" value="${esc(camera.url)}" placeholder="https://… oppure /percorso-locale"></label><button type="button" data-camera-remove aria-label="Elimina videocamera">×</button></div>`).join('') || '<p>Nessuna videocamera configurata.</p>'
}
