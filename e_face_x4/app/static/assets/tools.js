...
function renderShortcutAdmin(){
  $('#shortcuts-order-list').innerHTML = shortcutGroups.map((group, groupIndex) => {
    ...
    return `<section class="shortcut-admin-group" data-shortcut-category="${group.category}"><div class="shortcut-admin-head"><span>☰</span><strong>${shortcutLabels[group.category]}</strong><span class="shortcut-admin-move"><button type="button" data-shortcut-category-move="-1" ${groupIndex===0?'disabled':''}>↑</button><button type="button" data-shortcut-category-move="1" ${groupIndex===shortcutGroups.length-1?'disabled':''}>↓</button></span></div>${devices.map((device,index)=>`<label class="shortcut-admin-device" data-shortcut-device="${esc(device.id)}"><input type="checkbox" ${group.devices.includes(String(device.id))?'checked':''}><span><b>${esc(device.name)}</b><small>${esc(device.room||device.kind)}</small></span><span class="shortcut-admin-move"><button type="button" data-shortcut-device-move="-1" ${index===0?'disabled':''}>↑</button><button type="button" data-shortcut-device-move="1" ${index===devices.length-1?'disabled':''}>↓</button></span></label>`).join('')||'<small>Nessun dispositivo</small>'}</section>`
  }).join('')
}
...
$('#shortcuts-order-list').addEventListener('change',(event)=>{const row=event.target.closest('[data-shortcut-device]');if(!row)return;const group=shortcutGroups.find((item)=>item.category===row.closest('[data-shortcut-category]').dataset.shortcutCategory);const id=row.dataset.shortcutDevice;group.devices=event.target.checked?[...group.devices.filter((item)=>item!==id),id]:group.devices.filter((item)=>item!==id);renderShortcutAdmin()})
$('#shortcuts-order-list').addEventListener('click',(event)=>{const categoryButton=event.target.closest('[data-shortcut-category-move]');if(categoryButton){const category=categoryButton.closest('[data-shortcut-category]').dataset.shortcutCategory;const index=shortcutGroups.findIndex((item)=>item.category===category);const next=index+Number(categoryButton.dataset.shortcutCategoryMove);if(next>=0&&next<shortcutGroups.length)[shortcutGroups[index],shortcutGroups[next]]=[shortcutGroups[next],shortcutGroups[index]];renderShortcutAdmin();return}const deviceButton=event.target.closest('[data-shortcut-device-move]');if(!deviceButton)return;event.preventDefault();const row=deviceButton.closest('[data-shortcut-device]');const group=shortcutGroups.find((item)=>item.category===row.closest('[data-shortcut-category]').dataset.shortcutCategory);const ordered=[...row.closest('[data-shortcut-category]').querySelectorAll('[data-shortcut-device]')].map((item)=>item.dataset.shortcutDevice);const from=ordered.indexOf(row.dataset.shortcutDevice),to=from+Number(deviceButton.dataset.shortcutDeviceMove);if(to<0||to>=ordered.length)return;[ordered[from],ordered[to]]=[ordered[to],ordered[from]];group.devices=[...ordered.filter((id)=>group.devices.includes(id)),...group.devices.filter((id)=>!ordered.includes(id))];renderShortcutAdmin()})
