...
const homeWidgetPanel=document.createElement('section');homeWidgetPanel.className='media-config';homeWidgetPanel.hidden=true;homeWidgetPanel.innerHTML='<header><button type="button" data-home-widget-back>‹</button><div><h2>Home dinamica</h2></div></header><p>Componi la Home su una griglia a 12 colonne: trascina, dimensiona e guarda subito il risultato nell’anteprima.</p><label>Località meteo<input id="home-weather-location" maxlength="100" placeholder="Es. Torino, Piemonte"></label><label>Entità ultimo evento videosorveglianza<input id="home-camera-entity" placeholder="camera.nvr_32ch_ext_ultimo_evento"></label><fieldset class="home-todo-picker"><legend>Lista della spesa</legend><p>Scegli una delle liste esposte da e-Control.</p><div id="home-todo-options"><small>Caricamento liste…</small></div></fieldset><div id="home-widget-preview" class="home-widget-preview" aria-label="Anteprima disposizione Home"></div><div id="home-widget-list" class="shortcuts-order-list"></div><footer><button type="button" id="home-widget-save" class="user-save">SALVA HOME</button></footer>';document.querySelector('.tools-shell').append(homeWidgetPanel)
...
homeWidgetCard.addEventListener('click',async()=>{try{const [response,todoResponse]=await Promise.all([fetch(apiUrl('../api/user/appearance'),appearanceOptions({cache:'no-store'})),fetch(apiUrl('../api/home/todo/lists'),appearanceOptions({cache:'no-store'}))]);if(!response.ok)throw new Error(`HTTP ${response.status}`);const appearance=await response.json();const todo=todoResponse.ok?await todoResponse.json():{items:[]};homeWidgets=appearance.home_widgets||[];homeTodoEntity=appearance.home_todo_entity||todo.selected||'';$('#home-camera-entity').value=appearance.home_camera_entity||'camera.nvr_32ch_ext_ultimo_evento';$('#home-weather-location').value=appearance.home_weather_location||'';renderHomeTodoOptions(todo.items);renderHomeWidgetAdmin();homeWidgetPanel.hidden=false}catch(error){notice(error.message)}})
...
$('#home-widget-save').addEventListener('click',async()=>{try{syncHomeWidgets();homeTodoEntity=$('input[name="home-todo-entity"]:checked')?.value||'';if(homeWidgets.find(item=>item.id==='shopping_list')?.visible&&!homeTodoEntity)throw new Error('Scegli la lista da usare per Lista della spesa');const response=await fetch(apiUrl('../api/user/appearance'),appearanceOptions({method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({home_widgets:homeWidgets,home_camera_entity:$('#home-camera-entity').value.trim(),home_weather_location:$('#home-weather-location').value.trim(),...(homeTodoEntity?{home_todo_entity:homeTodoEntity}:{})})}));if(!response.ok)throw new Error((await response.json()).detail);notice('Home salvata · Lista della spesa aggiornata');homeWidgetPanel.hidden=true}catch(error){notice(error.message)}})
