...
$('#security-camera-add').addEventListener('click',()=>{securityCameras.push({id:newCameraId(),name:'',url:''});renderSecurityCameras();markSecurityDirty();$('#security-camera-list input')?.focus()})
$('#security-camera-list').addEventListener('input',event=>{const row=event.target.closest('[data-camera-index]');if(!row)return;const camera=securityCameras[Number(row.dataset.cameraIndex)];if(event.target.matches('[data-camera-name]'))camera.name=event.target.value;if(event.target.matches('[data-camera-url]'))camera.url=event.target.value.trim();markSecurityDirty()})
