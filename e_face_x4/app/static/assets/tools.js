...
function renderSecurityCameras() {
  $('#security-camera-list').innerHTML = securityCameras.map((camera,index)=>{const entity=/^camera\.[a-z0-9_]+$/.test(camera.url);return `<div class="security-camera-admin" data-camera-index="${index}"><button type="button" class="drag-handle" data-camera-drag aria-label="Trascina ${esc(camera.name||'videocamera')}">☰</button><div class="security-camera-admin-preview${entity?'':' invalid'}">${entity?`<img src="${apiUrl(`../api/user/security-camera-preview?entity=${encodeURIComponent(camera.url)}&v=${Date.now()}`)}" alt="Anteprima ${esc(camera.name||'videocamera')}" onerror="this.parentElement.classList.add('invalid');this.parentElement.insertAdjacentText('beforeend','Anteprima non disponibile')">`:'Inserisci entità camera.*'}</div><label>Nome<input data-camera-name maxlength="100" value="${esc(camera.name)}" placeholder="Es. Ingresso"></label><label>Link o entità e-Control<input data-camera-url maxlength="2000" value="${esc(camera.url)}" placeholder="camera.ingresso oppure https://…"></label><label>Modalità<select data-camera-mode><option value="snapshot" ${camera.mode==='video'?'':'selected'}>Fotogrammi</option><option value="video" ${camera.mode==='video'?'selected':''}>Video live + audio</option></select></label><button type="button" data-camera-save>SALVA E PROVA</button><button type="button" data-camera-remove aria-label="Elimina videocamera">×</button></div>`}).join('') || '<p>Nessuna videocamera configurata.</p>'
}
...
$('#security-camera-list').addEventListener('input',event=>{const row=event.target.closest('[data-camera-index]');if(!row)return;const camera=securityCameras[Number(row.dataset.cameraIndex)];if(event.target.matches('[data-camera-name]'))camera.name=event.target.value;if(event.target.matches('[data-camera-url]'))camera.url=event.target.value.trim();if(event.target.matches('[data-camera-mode]'))camera.mode=event.target.value;markSecurityDirty()})
async function saveSecurityCameras(message='Salvato ✓'){const response=await fetch(apiUrl('../api/user/appearance'),{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({security_order:orderedSecurity,security_cameras:securityCameras.map(camera=>({...camera,id:/^[A-Za-z0-9_-]{8,80}$/.test(camera.id||'')?camera.id:newCameraId(),name:camera.name.trim(),url:camera.url.trim()}))})});const data=await response.json();if(!response.ok)throw new Error(data.detail);securityCameras=data.security_cameras||securityCameras;renderSecurityCameras();securityStatus(message,'saved')}
