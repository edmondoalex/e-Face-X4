...
async function loadPlayers() {
  ...
  const detectedApps = data.skyq_apps || []
  $('#skyq-services').innerHTML = detectedApps.map(app => `<label class="${selectedApps.has(String(app.title).toLocaleLowerCase()) ? 'selected' : ''}"><input type="checkbox" value="${esc(app.title)}" ${selectedApps.has(String(app.title).toLocaleLowerCase()) ? 'checked' : ''}><img src="${apiUrl(`../api/skyq/apps/${encodeURIComponent(app.id)}/icon`)}" alt="" loading="lazy"><span>${esc(app.title)}</span></label>`).join('') || '<p class="skyq-apps-empty">Nessuna app letta. Verifica che Sky Q sia acceso, poi premi SALVA E VERIFICA DIRETTA.</p>'
...
function ensureSkyqPanel() {
  ...
  document.head.insertAdjacentHTML('beforeend', '<style>.skyq-form{margin:18px 0}.skyq-apps-head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-top:18px}.skyq-apps-actions{display:flex;gap:8px}.skyq-apps-actions button{padding:7px 11px;font-size:12px}.skyq-services{display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:10px;margin-top:10px}.skyq-services label{position:relative;display:flex;align-items:center;gap:10px;min-height:64px;padding:9px 12px;border:1px solid var(--line);border-radius:12px;background:rgba(255,255,255,.055);cursor:pointer;transition:.15s ease}.skyq-services label:active{transform:scale(.97)}.skyq-services label.selected{border-color:var(--cyan);background:rgba(75,210,240,.12);box-shadow:0 0 16px rgba(75,210,240,.12)}.skyq-services img{width:44px;height:44px;object-fit:contain;border-radius:9px;background:#181b1d}.skyq-services input{position:absolute;right:8px;top:8px;width:17px;height:17px;accent-color:var(--cyan)}.skyq-services span{padding-right:15px;font-weight:700}.skyq-apps-empty{grid-column:1/-1;color:var(--muted)}</style>')
