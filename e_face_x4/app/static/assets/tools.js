...
const homeWidgetPanel=document.createElement('section');homeWidgetPanel.className='media-config';homeWidgetPanel.hidden=true;homeWidgetPanel.innerHTML='<header><button type="button" data-home-widget-back>‹</button><div><h2>Home dinamica</h2></div></header><p>Trascina i widget nell’ordine desiderato, scegli la dimensione e nascondi ciò che non ti serve.</p><label>Località meteo<input id="home-weather-location" maxlength="100" placeholder="Es. Torino, Piemonte"></label><label>Entità ultimo evento videosorveglianza<input id="home-camera-entity" placeholder="camera.nvr_32ch_ext_ultimo_evento"></label><div id="home-widget-list" class="shortcuts-order-list"></div><footer><button type="button" id="home-widget-save" class="user-save">SALVA HOME</button></footer>';document.querySelector('.tools-shell').append(homeWidgetPanel)
let homeWidgets=[]
function renderHomeWidgetAdmin(){ $('#home-widget-list').innerHTML=homeWidgets.map((item)=>{const label=homeWidgetLabels[item.id]||[item.id,''],image=['camera_event','doorbell','motion'].includes(item.id),height=image?`<select data-widget-height aria-label="Altezza ${label[0]}"><option value="short" ${item.height==='short'?'selected':''}>Bassa</option><option value="standard" ${!item.height||item.height==='standard'?'selected':''}>Media</option><option value="tall" ${item.height==='tall'?'selected':''}>Alta</option></select>`:'';return `<div class="shortcut-admin-device home-widget-admin" data-home-widget-admin="${item.id}"><button type="button" class="shortcut-drag-handle" data-home-widget-drag aria-label="Trascina ${label[0]}">☰</button><span><b>${label[0]}</b><small>${label[1]}</small></span><select data-widget-size aria-label="Larghezza ${label[0]}"><option value="compact" ${item.size==='compact'?'selected':''}>Compatto</option><option value="standard" ${item.size==='standard'?'selected':''}>Medio</option><option value="wide" ${item.size==='wide'?'selected':''}>Largo</option></select>${height}<input type="checkbox" aria-label="Mostra ${label[0]}" ${item.visible?'checked':''}></div>`}).join('') }
function syncHomeWidgets(){homeWidgets=[...$('#home-widget-list').querySelectorAll('[data-home-widget-admin]')].map((row)=>({id:row.dataset.homeWidgetAdmin,visible:row.querySelector('input').checked,size:row.querySelector('[data-widget-size]').value,height:row.querySelector('[data-widget-height]')?.value||'standard'}))}
