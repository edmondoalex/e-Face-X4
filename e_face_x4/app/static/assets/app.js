...
async function openHomeEventViewer(card){const image=card.querySelector('img');if(!image?.src||card.classList.contains('unavailable'))return;const dialog=$('#home-event-dialog'),viewer=$('#home-event-dialog-image');let video=$('#home-event-dialog-video');if(!video){video=document.createElement('video');video.id='home-event-dialog-video';video.controls=true;video.playsInline=true;video.autoplay=true;video.style.cssText='width:100%;height:100%;object-fit:contain';viewer.parentElement.append(video)}stopHomeEventRefresh();video.pause();video.removeAttribute('src');video.hidden=true;viewer.hidden=false;viewer.src=image.src;$('#home-event-dialog-source').textContent=card.querySelector('small')?.textContent||'IMMAGINE';$('#home-event-dialog-title').textContent=card.querySelector('strong')?.textContent||image.alt||'Evento';if(card.dataset.cameraMode==='video'){try{const response=await fetch(apiUrl(`api/security/cameras/${encodeURIComponent(card.dataset.cameraId)}/stream`),{cache:'no-store'}),data=await response.json();if(!response.ok)throw new Error(data.detail||`HTTP ${response.status}`);viewer.hidden=true;video.hidden=false;video.src=data.url;await video.play()}catch(error){notify(`Video live non disponibile: ${error.message}`)}}else if(card.matches('[data-security-camera-entity]')){refreshOpenCamera(image.src);homeEventRefreshTimer=setInterval(()=>refreshOpenCamera(image.src),1000)}if(!dialog.open)dialog.showModal()}
