...
document.head.insertAdjacentHTML('beforeend', '<link rel="stylesheet" href="assets/media-x4.css?v=2.21.156">')
...
function renderMediaExperience(devices) {
  ...
  const channelLogo = selected.skyq && selected.skyq_channel_fingerprint ? `<span class="media-skyq-channel-logo"><img src="${apiUrl(`api/media/${encodeURIComponent(selected.registry_id)}/artwork?fingerprint=${encodeURIComponent(selected.skyq_channel_fingerprint)}`)}" alt="${esc(selected.channel || '')}" onerror="this.parentElement.classList.add('failed');this.hidden=true"><i class="mdi-mask" style="${mdiStyle(mainIcon, 'television')}"></i></span>` : ''
  const sourceGlyph = channelLogo ? `<span class="device-glyph media-skyq-logo-stack">${activeMediaSourceMarkup(selected, 'media-skyq-source-logo', mainIcon)}${channelLogo}</span>` : activeMediaSourceMarkup(selected, 'device-glyph', mainIcon)
