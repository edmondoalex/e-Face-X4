...
document.head.insertAdjacentHTML('beforeend', '<link rel="stylesheet" href="assets/media-x4.css?v=2.21.119">')
...
const weatherMeta=(code)=>{code=Number(code);if(code===0)return['Sereno','mdi:weather-sunny','sunny'];if([1,2].includes(code))return['Poco nuvoloso','mdi:weather-partly-cloudy','cloudy'];if(code===3)return['Nuvoloso','mdi:weather-cloudy','cloudy'];if([45,48].includes(code))return['Nebbia','mdi:weather-fog','fog'];if(code>=51&&code<=67)return['Pioggia','mdi:weather-rainy','rain'];if(code>=71&&code<=77)return['Neve','mdi:weather-snowy','snow'];if(code>=80&&code<=82)return['Rovesci','mdi:weather-pouring','rain'];if(code>=85&&code<=86)return['Neve','mdi:weather-snowy','snow'];if(code>=95)return['Temporale','mdi:weather-lightning-rainy','storm'];return['Variabile','mdi:weather-partly-cloudy','cloudy']}
function renderWeather(data){const current=data.current||{},daily=data.daily||{},meta=weatherMeta(current.weather_code),card=$('#home-weather-widget');card.dataset.weather=meta[2];$('#home-weather-place').textContent=[data.name,data.area].filter(Boolean).join(' · ');$('#home-weather-clock').textContent=new Intl.DateTimeFormat('it-IT',{hour:'2-digit',minute:'2-digit',timeZone:data.timezone||undefined}).format(new Date());$('#home-weather-icon').style.cssText=mdiStyle(meta[1],'weather-partly-cloudy');$('#home-weather-state').textContent=meta[0];$('#home-weather-temperature').textContent=`${Math.round(Number(current.temperature_2m))}°`;$('#home-weather-detail').textContent=`Percepita ${Math.round(Number(current.apparent_temperature))}° · UR ${Math.round(Number(current.relative_humidity_2m))}% · Vento ${Math.round(Number(current.wind_speed_10m))} km/h`;const days=daily.time||[];$('#home-weather-forecast').innerHTML=days.slice(0,5).map((date,index)=>{const item=weatherMeta((daily.weather_code||[])[index]);const day=new Intl.DateTimeFormat('it-IT',{weekday:'short'}).format(new Date(`${date}T12:00:00`)).replace('.','').toUpperCase();return `<div><small>${day}</small><i class="mdi-mask" style="${mdiStyle(item[1],'weather-partly-cloudy')}"></i><span><b>${Math.round(Number((daily.temperature_2m_max||[])[index]))}°</b> ${Math.round(Number((daily.temperature_2m_min||[])[index]))}°</span></div>`}).join('')}
