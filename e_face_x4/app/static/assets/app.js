...
document.head.insertAdjacentHTML('beforeend', '<link rel="stylesheet" href="assets/media-x4.css?v=2.21.166">')
...
function renderSecurityDevices(devices) {
  ...
  const cameraCards = currentSecurityCameras.map((camera) => {
    ...
    return entity ? `<button type="button" class="security-camera security-camera-preview" data-security-camera-entity="${esc(camera.url)}"><img src="${apiUrl(`api/security/cameras/${encodeURIComponent(camera.id)}/image?v=${Date.now()}`)}" alt="Anteprima ${esc(camera.name)}" onload="this.closest('.security-camera').classList.remove('unavailable')" onerror="this.closest('.security-camera').classList.add('unavailable')"><span><small>E-CONTROL</small><strong>${esc(camera.name)}</strong></span></button>` : `<button type="button" class="security-camera" data-security-camera-url="${esc(camera.url)}"><span class="mdi-mask" style="${mdiStyle('mdi:cctv','cctv')}"></span><span><strong>${esc(camera.name)}</strong><small>APRI VIDEOCAMERA</small></span><i class="mdi-mask" style="${mdiStyle('mdi:open-in-new','open-in-new')}"></i></button>`
  }).join('')
