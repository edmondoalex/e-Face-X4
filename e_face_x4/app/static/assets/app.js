...
async function loadAlexaAgenda(){
  ...
  try{const response=await fetch(apiUrl('api/home/alexa/agenda'),{cache:'no-store'});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.detail||`HTTP ${response.status}`);const devices=data.devices||[],selected=select.value,currentSource=sourceSelect.value;select.innerHTML='<option value="">Scegli Echo</option>'+devices.map(device=>`<option value="${escAttribute(device.device_id)}">${esc(device.name)}</option>`).join('');if(devices.some(device=>device.device_id===selected))select.value=selected;else if(devices.length===1)select.value=devices[0].device_id;sourceSelect.innerHTML=(data.sources||[]).map(source=>`<option value="${escAttribute(source.id)}">${esc(source.name)}</option>`).join('');sourceSelect.value=(data.sources||[]).some(source=>source.id===currentSource)?currentSource:data.selected_source||'alexa';syncAgendaSource();const labels={alarm:'Prossima sveglia',timer:'Prossimo timer',reminder:'Prossimo promemoria'},icons={alarm:'◷',timer:'◴',reminder:'◆'};const alexaRows=devices.flatMap(device=>Object.entries(device.agenda||{}).filter(([,event])=>hasAlexaAgendaEvent(event.state)).map(([kind,event])=>`<article class="alexa-agenda-event"><i>${icons[kind]||'◷'}</i><div><b>${esc(labels[kind]||kind)} · ${esc(device.name)}</b><span>${esc(formatAlexaAgendaState(event.state))}</span></div></article>`));const internalRows=(data.internal||[]).filter(event=>hasAlexaAgendaEvent(event.start)).map(event=>`<article class="alexa-agenda-event"><i>${icons[event.kind]||'◆'}</i><div><b>${esc(event.summary)}</b><span>${esc(formatAlexaAgendaState(event.start))} · Agenda e-Control</span></div></article>`);setAgendaNavCount(alexaRows.length+internalRows.length);items.innerHTML=[...internalRows,...alexaRows].join('')||'<p class="alexa-agenda-empty">Nessuna sveglia, timer o promemoria attivo.</p>'
    if(sourceSelect.value==='alexa')await loadAlexaNotificationItems()
  }catch(error){items.innerHTML=`<p class="alexa-agenda-empty">${esc(error.message)}</p>`}
}
async function loadAlexaNotificationItems(){const items=$('#alexa-agenda-items');items.innerHTML='<p class="alexa-agenda-empty">Lettura eventi Alexa…</p>';try{const response=await fetch(apiUrl('api/home/alexa/agenda/items'),{cache:'no-store'}),data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.detail||`HTTP ${response.status}`);const labels={alarm:'Sveglia',timer:'Timer',reminder:'Promemoria'},icons={alarm:'◷',timer:'◴',reminder:'◆'},active=(data.items||[]).filter(item=>item.status==='ON');setAgendaNavCount(active.length);items.innerHTML=active.map(item=>{const when=item.scheduled_time?formatAlexaAgendaState(item.scheduled_time):item.alarm_time?formatAlexaAgendaState(new Date(Number(item.alarm_time)).toISOString()):item.remaining_ms?`${Math.ceil(Number(item.remaining_ms)/60000)} minuti`:'Programma attivo',device=item.device_name?` · ${item.device_name}`:'';return `<article class="alexa-agenda-event"><i>${icons[item.kind]||'◷'}</i><div><b>${esc(item.label||labels[item.kind]||'Evento Alexa')}</b><span>${esc(labels[item.kind]||item.kind)} · ${esc(when)}${esc(device)}</span></div><button type="button" data-alexa-delete="${escAttribute(item.id)}" data-alexa-device="${escAttribute(item.device_id||'')}" aria-label="Elimina ${escAttribute(labels[item.kind]||'evento')}">×</button></article>`}).join('')||'<p class="alexa-agenda-empty">Nessun evento Alexa attivo.</p>'}catch(error){items.innerHTML=`<p class="alexa-agenda-empty">${esc(error.message)}</p>`}}
...
$('#alexa-agenda-items')?.addEventListener('click',async event=>{const button=event.target.closest('[data-alexa-delete]');if(!button)return;const deviceId=button.dataset.alexaDevice||$('#alexa-agenda-device').value;if(!deviceId||!confirm('Eliminare questo evento da Alexa?'))return;button.disabled=true;try{const response=await fetch(apiUrl(`api/home/alexa/agenda/items/${encodeURIComponent(button.dataset.alexaDelete)}?device_id=${encodeURIComponent(deviceId)}`),{method:'DELETE'}),data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.detail||`HTTP ${response.status}`);await loadAlexaNotificationItems()}catch(error){$('#alexa-agenda-status').textContent=`Errore: ${error.message}`;button.disabled=false}})
