...
async function loadAlexaNotificationItems(agendaData=null){
  ...
  try{
    ...
    setAgendaNavCount(alexaRows.length+internalRows.length);items.innerHTML=[...alexaRows,...internalRows].join('')||'<p class="alexa-agenda-empty">Nessun evento Alexa o e-Control attivo.</p>'
  }catch(error){items.innerHTML=`<p class="alexa-agenda-empty">${esc(error.message)}</p>`}
}
...
$('#alexa-agenda-items')?.addEventListener('click',async event=>{const button=event.target.closest('[data-alexa-delete],[data-internal-agenda-delete]');if(!button)return;const internalId=button.dataset.internalAgendaDelete,deviceId=button.dataset.alexaDevice||$('#alexa-agenda-device').value;if(!internalId&&!deviceId)return;if(!confirm(`Eliminare questo evento ${internalId?'e-Control':'Alexa'}?`))return;button.disabled=true;try{const url=internalId?`api/home/alexa/agenda/internal/${encodeURIComponent(internalId)}`:`api/home/alexa/agenda/items/${encodeURIComponent(button.dataset.alexaDelete)}?device_id=${encodeURIComponent(deviceId)}`;const response=await fetch(apiUrl(url),{method:'DELETE'}),data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.detail||`HTTP ${response.status}`);await loadAlexaAgenda()}catch(error){$('#alexa-agenda-status').textContent=`Errore: ${error.message}`;button.disabled=false}})
