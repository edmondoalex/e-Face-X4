...
async function refreshHomeAgendaWidget(){try{const response=await fetch(apiUrl('api/home/alexa/agenda'),{cache:'no-store'});if(!response.ok)throw new Error();const data=await response.json(),source=(data.sources||[]).find(item=>item.id===data.selected_source),events=[];(data.internal||[]).forEach(item=>events.push({label:item.summary,state:item.start}));(data.devices||[]).forEach(device=>Object.entries(device.agenda||{}).forEach(([kind,item])=>{if(item.state&&!['unknown','unavailable'].includes(item.state))events.push({label:{alarm:'Sveglia',timer:'Timer',reminder:'Promemoria'}[kind]||kind,state:item.state})}));events.sort((a,b)=>new Date(a.state)-new Date(b.state));$('#home-agenda-title').textContent=source?.name||'Agenda';$('#home-agenda-preview').textContent=events[0]?`${events[0].label} · ${formatAlexaAgendaState(events[0].state)}`:'Nessun prossimo evento'}catch{$('#home-agenda-title').textContent='Agenda';$('#home-agenda-preview').textContent='Momentaneamente non disponibile'}}
...
async function loadAlexaAgenda(){
  ...
  try{const response=await fetch(apiUrl('api/home/alexa/agenda'),{cache:'no-store'});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.detail||`HTTP ${response.status}`);const devices=data.devices||[],selected=select.value,currentSource=sourceSelect.value;select.innerHTML='<option value="">Scegli Echo</option>'+devices.map(device=>`<option value="${escAttribute(device.device_id)}">${esc(device.name)}</option>`).join('');if(devices.some(device=>device.device_id===selected))select.value=selected;else if(devices.length===1)select.value=devices[0].device_id;sourceSelect.innerHTML=(data.sources||[]).map(source=>`<option value="${escAttribute(source.id)}">${esc(source.name)}</option>`).join('');sourceSelect.value=(data.sources||[]).some(source=>source.id===currentSource)?currentSource:data.selected_source||'alexa';syncAgendaSource();const labels={alarm:'Prossima sveglia',timer:'Prossimo timer',reminder:'Prossimo promemoria'},icons={alarm:'◷',timer:'◴',reminder:'◆'};const alexaRows=devices.flatMap(device=>Object.entries(device.agenda||{}).filter(([,event])=>hasAlexaAgendaEvent(event.state)).map(([kind,event])=>`<article class="alexa-agenda-event"><i>${icons[kind]||'◷'}</i><div><b>${esc(labels[kind]||kind)} · ${esc(device.name)}</b><span>${esc(formatAlexaAgendaState(event.state))}</span></div></article>`));const internalRows=(data.internal||[]).filter(event=>hasAlexaAgendaEvent(event.start)).map(event=>`<article class="alexa-agenda-event"><i>${icons[event.kind]||'◆'}</i><div><b>${esc(event.summary)}</b><span>${esc(formatAlexaAgendaState(event.start))} · Agenda e-Control</span></div></article>`);items.innerHTML=[...internalRows,...alexaRows].join('')||'<p class="alexa-agenda-empty">Nessuna sveglia, timer o promemoria attivo.</p>'
    if(sourceSelect.value==='alexa'&&select.value)await loadAlexaNotificationItems(select.value)
  }catch(error){items.innerHTML=`<p class="alexa-agenda-empty">${esc(error.message)}</p>`}
}
async function loadAlexaNotificationItems(deviceId){const items=$('#alexa-agenda-items');items.innerHTML='<p class="alexa-agenda-empty">Lettura eventi Alexa…</p>';try{const response=await fetch(apiUrl(`api/home/alexa/agenda/items?device_id=${encodeURIComponent(deviceId)}`),{cache:'no-store'}),data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.detail||`HTTP ${response.status}`);const labels={alarm:'Sveglia',timer:'Timer',reminder:'Promemoria'},icons={alarm:'◷',timer:'◴',reminder:'◆'};items.innerHTML=(data.items||[]).filter(item=>item.status==='ON').map(item=>{const when=item.scheduled_time?formatAlexaAgendaState(item.scheduled_time):item.alarm_time?formatAlexaAgendaState(new Date(Number(item.alarm_time)).toISOString()):item.remaining_ms?`${Math.ceil(Number(item.remaining_ms)/60000)} minuti`:'Programma attivo';return `<article class="alexa-agenda-event"><i>${icons[item.kind]||'◷'}</i><div><b>${esc(item.label||labels[item.kind]||'Evento Alexa')}</b><span>${esc(labels[item.kind]||item.kind)} · ${esc(when)}</span></div><button type="button" data-alexa-delete="${escAttribute(item.id)}" aria-label="Elimina ${escAttribute(labels[item.kind]||'evento')}">×</button></article>`}).join('')||'<p class="alexa-agenda-empty">Nessun evento Alexa attivo.</p>'}catch(error){items.innerHTML=`<p class="alexa-agenda-empty">${esc(error.message)}</p>`}}
...
$('#alexa-agenda-device')?.addEventListener('change',event=>{if(event.target.value)loadAlexaNotificationItems(event.target.value)})
$('#alexa-agenda-items')?.addEventListener('click',async event=>{const button=event.target.closest('[data-alexa-delete]');if(!button)return;const deviceId=$('#alexa-agenda-device').value;if(!deviceId||!confirm('Eliminare questo evento da Alexa?'))return;button.disabled=true;try{const response=await fetch(apiUrl(`api/home/alexa/agenda/items/${encodeURIComponent(button.dataset.alexaDelete)}?device_id=${encodeURIComponent(deviceId)}`),{method:'DELETE'}),data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.detail||`HTTP ${response.status}`);await loadAlexaNotificationItems(deviceId)}catch(error){$('#alexa-agenda-status').textContent=`Errore: ${error.message}`;button.disabled=false}})
