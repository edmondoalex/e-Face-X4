...
async function refreshHomeAgendaWidget(){try{const response=await fetch(apiUrl('api/home/alexa/agenda'),{cache:'no-store'});if(!response.ok)throw new Error();const data=await response.json(),source=(data.sources||[]).find(item=>item.id===data.selected_source),events=[];(data.internal||[]).filter(item=>hasAlexaAgendaEvent(item.start)).forEach(item=>events.push({label:item.summary,state:item.start}));(data.devices||[]).forEach(device=>Object.entries(device.agenda||{}).forEach(([kind,item])=>{if(hasAlexaAgendaEvent(item.state))events.push({label:{alarm:'Sveglia',timer:'Timer',reminder:'Promemoria'}[kind]||kind,state:item.state})}));events.sort((a,b)=>new Date(a.state)-new Date(b.state));setAgendaNavCount(events.length);$('#home-agenda-title').textContent=source?.name||'Agenda';$('#home-agenda-preview').textContent=events[0]?`${events[0].label} · ${formatAlexaAgendaState(events[0].state)}`:'Nessun prossimo evento'}catch{$('#home-agenda-title').textContent='Agenda';$('#home-agenda-preview').textContent='Momentaneamente non disponibile'}}
