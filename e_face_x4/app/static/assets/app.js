...
async function loadAlexaNotificationItems(deviceId){const items=$('#alexa-agenda-items');items.innerHTML='<p class="alexa-agenda-empty">Lettura eventi Alexa…</p>';try{const response=await fetch(apiUrl(`api/home/alexa/agenda/items?device_id=${encodeURIComponent(deviceId)}`),{cache:'no-store'}),data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.detail||`HTTP ${response.status}`);const labels={alarm:'Sveglia',timer:'Timer',reminder:'Promemoria'},icons={alarm:'◷',timer:'◴',reminder:'◆'};items.innerHTML=(data.items||[]).filter(item=>item.status==='ON').map(item=>{const when=item.alarm_time?formatAlexaAgendaState(new Date(Number(item.alarm_time)).toISOString()):item.remaining_ms?`${Math.ceil(Number(item.remaining_ms)/60000)} minuti`:'Programma attivo';return `<article class="alexa-agenda-event"><i>${icons[item.kind]||'◷'}</i><div><b>${esc(item.label||labels[item.kind]||'Evento Alexa')}</b><span>${esc(labels[item.kind]||item.kind)} · ${esc(when)}</span></div><button type="button" data-alexa-delete="${escAttribute(item.id)}" aria-label="Elimina ${escAttribute(labels[item.kind]||'evento')}">×</button></article>`}).join('')||'<p class="alexa-agenda-empty">Nessun evento Alexa attivo.</p>'}catch(error){items.innerHTML=`<p class="alexa-agenda-empty">${esc(error.message)}</p>`}}
