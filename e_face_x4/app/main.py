...
VERSION = os.environ.get("EFACE_VERSION", "2.21.169")
...
def create_app() -> FastAPI:
    ...
    async def home_assistant_camera_stream(entity_id: str) -> str:
        ...
        try:
            async with websockets.connect("ws://supervisor/core/websocket", open_timeout=8) as socket:
                ...
                if not result.get("success") or not url.startswith("/api/hls/"): raise RuntimeError("Flusso HLS non disponibile")
                return url
        except (OSError, asyncio.TimeoutError, RuntimeError, websockets.exceptions.WebSocketException) as exc:
            raise HTTPException(status_code=502, detail=str(exc)) from exc
    ...
    def themed_page(filename: str, body_class: str = "") -> str:
        ...
        page = page.replace("app.css?v=2.21.84", "app.css?v=2.21.169")
        page = page.replace("home-status.css?v=2.20.20", "home-status.css?v=2.21.169")
        page = page.replace("tools.js?v=2.21.1", "tools.js?v=2.21.169")
        page = page.replace("tools-user.css?v=2.21.163", "tools-user.css?v=2.21.169")
        ...
        page = page.replace("tools-dashboard.js?v=2.21.42", "tools-dashboard.js?v=2.21.169")
        page = page.replace("tools-dashboard.css?v=2.20.36", "tools-dashboard.css?v=2.21.169")
    ...
    @app.get("/api/installer/skyq/inventory")
    async def installer_skyq_inventory(request: Request, refresh: bool = False) -> dict:
        require_installer(request)
        try:
            return await skyq_connector.inventory(skyq_settings.load(), refresh=refresh)
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
        except (ConnectionError, OSError, TypeError) as exc:
            raise HTTPException(status_code=502, detail=str(exc)) from exc

    @app.get("/api/installer/skyq/guide")
    async def installer_skyq_guide(request: Request, sid: str, day: str) -> dict:
        require_installer(request)
        try:
            return {"items": await skyq_connector.guide(skyq_settings.load(), sid, day), "sid": sid, "day": day}
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
        except (ConnectionError, OSError, TypeError) as exc:
            raise HTTPException(status_code=502, detail=str(exc)) from exc
    ...
    @app.get("/api/skyq/apps/{app_id}/icon", include_in_schema=False)
    async def skyq_app_icon(app_id: str) -> Response:
        ...
        try:
            ...
            raise HTTPException(status_code=502, detail="Icona Sky Q non raggiungibile")
        if upstream.status_code != 200:
            raise HTTPException(status_code=upstream.status_code, detail="Icona Sky Q non disponibile")
        media_type = artwork_media_type(upstream.headers.get("content-type", ""), upstream.content)
