...
class Control4MediaConnector(Connector):
    ...
    async def command(self, registry_id: str, operation: str, value: Any = None) -> dict[str, Any]:
        ...
        if operation == "media_play": await room.set_play()
            ...
            if not members or len(members) > 63:
                raise ValueError("Seleziona almeno una stanza Control4 da aggiungere")
            ...
            if len(set(requested)) != len(requested) or any(item not in valid or item == registry_id for item in requested):
                raise ValueError("Stanza Control4 non valida")
            source_id = await director.get_item_variable_value(room_id, "PLAYING_AUDIO_DEVICE")
            if not str(source_id or "0").isdigit() or int(source_id) <= 0:
                raise ValueError("Avvia prima una sorgente audio nella stanza principale")
            results = await asyncio.gather(*(C4Room(director, int(item.removeprefix("c4room:"))).set_audio_source(int(source_id)) for item in requested), return_exceptions=True)
            failures = [requested[index] for index, result in enumerate(results) if isinstance(result, Exception)]
            if failures:
                raise RuntimeError(f"Aggiunta non riuscita per {len(failures)} stanze")
        elif operation == "media_unjoin":
            await room.set_room_off()
...
def normalize_control4_groups(items: list[dict[str, Any]], variables: Any) -> list[dict[str, Any]]:
    ...
    queues = queue_value.get("queues", {}).get("queue", []) if isinstance(queue_value, dict) else []
    if isinstance(queues, dict):
        queues = [queues]
    groups = []
    for queue in queues if isinstance(queues, list) else []:
        if not isinstance(queue, dict):
            continue
        room_values = queue.get("rooms", {}).get("id", []) if isinstance(queue.get("rooms"), dict) else []
        if not isinstance(room_values, list):
            room_values = [room_values]
        members = [f"c4room:{room_id}" for room_id in room_values if f"c4room:{room_id}" in player_ids]
        if len(members) < 2:
            continue
        group = {"group_id": f"c4queue:{queue.get('id')}", "name": str(queue.get("name") or "Sessione audio"), "member_registry_ids": members, "completeness": "complete", "resource_revision": int(queue.get("id") or 0)}
        groups.append(group)
        for player in items:
