...
La configurazione upstream usa `host_network: true` e `ingress_port: 8088`. Un server HTTP legato solo a `127.0.0.1` **non è raggiungibile** dal container e-Face; non allargare il bind a `0.0.0.0` con il solo Bearer in chiaro. Serve un canale autenticato e cifrato o mediato in modo ristretto, da progettare e testare prima del packaging. Non far partire contemporaneamente una variante Asterisk e l'add-on esistente: condividerebbero le porte SIP/HTTP.
