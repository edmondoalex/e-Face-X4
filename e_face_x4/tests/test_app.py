...
from app.main import artwork_media_type, camera_event_timestamp, create_app, wiim_is_standalone_room
...
def test_nvr_event_uses_latest_photo_timestamp_not_stale_idle_state():
    state = {"state": "idle", "last_changed": "2026-09-18T12:05:05Z", "last_updated": "2026-09-19T13:32:54Z", "attributes": {"Data ultima foto": 1789824773.9884489}}
    assert camera_event_timestamp(state) == "2026-09-19T13:32:53.988449+00:00"
    state["attributes"]["Data ultima foto"] = None
    assert camera_event_timestamp(state) == state["last_updated"]
...
def test_scenario_studio_tutorial_is_collapsible_and_uses_econtrol_labels() -> None:
    script = (Path(__file__).resolve().parents[1] / "app/static/assets/scenario-tools.js").read_text(encoding="utf-8")
    assert '<details class="scene-tutorial"><summary>' in script
    for phrase in ("COME SI CREA UNO SCENARIO", "Apertura graduale", "RUN", "ON/OFF", "offset", "pulsante e-Control"):
        assert phrase in script
    assert "Home Assistant" not in script
    assert "Pulsante HA" not in script


def test_scenario_studio_uses_viewport_overlay_and_tools_restore_only_on_refresh() -> None:
    static = Path(__file__).resolve().parents[1] / "app/static"
    page = (static / "tools.html").read_text(encoding="utf-8")
    layout = (static / "assets/scenario-layout-fix.css").read_text(encoding="utf-8")
    dashboard = (static / "assets/tools-dashboard.js").read_text(encoding="utf-8")
    assert 'scenario-layout-fix.css?v=2.21.220' in page
    assert '#scenarios-config' in layout and 'width: 100vw !important' in layout
    assert "navigation?.type !== 'reload'" in dashboard
    assert "sessionStorage.removeItem('eface-tools-restore')" in dashboard
...
def test_health() -> None:
    ...
    assert response.json()["version"] == "2.21.266"
...
def test_intercom_is_in_sidebar_with_embedded_view() -> None:
    ...
    assert "const currentVersion = '2.21.266'" in client_script
    ...
    assert "Notifica inviata" in client_script
    assert "const videoDestination = button.dataset.videoCapable === 'true' || !!externalStation" in client_script
    assert 'id="call-doorbird" data-dial-extension="8201" data-video-capable="true"' in intercom_page
    assert "if (externalStation) showCallDoorbirdVideo(externalStation)" in client_script
    assert "const includeVideo = videoDestination && !externalStation && videoEnabled" in client_script
    assert "$('#intercom-call-panel').hidden = false" in client_script
    assert "setTimeout(resolve, 4500)" in client_script
    assert client_script.count("$('#intercom-video-panel').scrollIntoView({behavior:'smooth', block:'start'})") == 4
...
def test_intercom_dashboard_stores_only_local_settings(monkeypatch, tmp_path) -> None:
    ...
    assert "tools-dashboard.js?v=2.21.266" in page
    assert "tools-dashboard.css?v=2.21.266" in page
    assert "tools.js?v=2.21.266" in page
    assert "organization-tools.js?v=2.21.266" in page
    ...
    assert "data-shortcut-drag=\"device\"" in tools_js
    assert "pointermove" in tools_js and "finishShortcutDrag" in tools_js
    assert "Larghezza" in tools_js and "Altezza" in tools_js
    assert "data-preview-height" in tools_js
    assert "L ${spans[item.size]||6}/12 · H" in tools_js
    home = client.get("/").text
    assert '<b>Accesi</b>' not in home
    assert 'id="light-on-filter"' in home
    assert "backgrounds.css?v=2.21.43" in home
    assert "app.js?v=2.21.266" in home
    ...
    assert "refreshHomeEventNow('camera')" in app_js
    ...
    assert "setAgendaNavCount(rows.length)" in app_js
...
def test_admin_doorbird_configuration_is_read_only_and_reveal_requires_reauth(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("EFACE_AUTH_DIR", str(tmp_path / "auth"))
    monkeypatch.setenv("EFACE_CREDENTIAL_INVENTORY", str(tmp_path / "inventory.json"))
    from app.user_auth import create_admin
    from app import credential_inventory, doorbird_api
    ...
    credential_inventory.save("doorbird", "doorbird-admin", "doorbird-secret")
    client = TestClient(create_app())
    endpoint = "/api/admin/intercom/doorbird/configuration"
    assert client.get(endpoint).status_code == 401
    assert client.post("/api/auth/login", json={"username": "admin", "password": "password-admin-lunga"}).status_code == 200
    calls = []

    async def fake_configuration(host, port, username, password, asterisk_host, ring_extension, reveal=False):
        calls.append((host, port, username, password, asterisk_host, ring_extension, reveal))
        return {"sip": {"PASSWORD": "visible" if reveal else "configurato"}}

    monkeypatch.setattr(doorbird_api, "configuration", fake_configuration)
    hidden = client.get(endpoint)
    assert hidden.json()["sip"]["PASSWORD"] == "configurato"
    assert hidden.headers["cache-control"] == "no-store, private"
    reveal = f"{endpoint}/reveal"
    assert client.post(reveal, json={"admin_password": "wrong"}).status_code == 403
    visible = client.post(reveal, json={"admin_password": "password-admin-lunga"})
    assert visible.json()["sip"]["PASSWORD"] == "visible"
...
def test_doorbird_image_requires_login_and_keeps_credential_server_side(monkeypatch, tmp_path) -> None:
    ...
    assert cached.content == b"\xff\xd8\xffimage"
...
def test_tools_page_starts_with_selected_background_and_card_theme(monkeypatch, tmp_path) -> None:
    ...
    assert 'ui-theme-contract.css?v=2.21.266' in home
    assert 'app.js?v=2.21.266' in home
...
def test_user_appearance_persists_room_order_and_glow(monkeypatch, tmp_path) -> None:
    ...
    assert {key: appearance[key] for key in ("card_glow", "room_order", "security_order", "security_cameras", "shortcuts")} == {"card_glow": True, "room_order": [], "security_order": ["scenarios", "areas", "zones", "sensors", "locks", "cameras"], "security_cameras": [], "shortcuts": []}
    assert [item["id"] for item in appearance["home_widgets"]] == ["overview", "states", "live", "weather", "camera_event", "doorbell", "motion", "rooms", "lights_now", "routine_pulse", "shopping_list", "agenda", "pet_feeder", "room_pulse"]
    assert appearance["home_widgets"] == [
    ...
    response = client.put("/api/user/appearance", json={"card_glow": False, "room_order": ["Sala", "Ufficio Alex"], "security_order": ["locks", "zones", "sensors", "areas", "scenarios", "cameras"], "security_cameras": cameras, "shortcuts": shortcuts, "home_widgets": home_widgets, "home_camera_entity": "camera.nvr_32ch_ext_ultimo_evento", "home_weather_location": "Torino"})
    assert response.status_code == 200
    navigation = [{"id": key, "visible": True} for key in ["watch", "listen", "intercom", "lights", "extra", "scenarios", "covers", "comfort", "sensors", "heating", "energy", "security", "shopping", "alexa-agenda"]]
    expanded_widgets = [*home_widgets, *[{"id": item, "visible": False, "size": "wide" if item == "room_pulse" else "standard", "height": "standard"} for item in ("room_pulse", "lights_now", "routine_pulse", "shopping_list", "agenda", "pet_feeder")]]
    assert client.get("/api/user/appearance").json() == {"card_glow": False, "room_order": ["Sala", "Ufficio Alex"], "security_order": ["locks", "zones", "sensors", "areas", "scenarios", "cameras"], "security_cameras": cameras, "shortcuts": shortcuts, "device_organization": {}, "navigation_items": navigation, "home_widgets": expanded_widgets, "home_camera_entity": "camera.nvr_32ch_ext_ultimo_evento", "home_weather_location": "Torino", "home_todo_entity": "", "home_agenda_source": "alexa"}
    ...
    assert client.get("/api/user/appearance").json()["navigation_items"] == navigation
    assert client.put("/api/user/appearance", json={"navigation_items": navigation[:-1]}).status_code == 400
    ...
    assert client.put("/api/user/appearance", json={"security_order": ["locks", "zones", "zones", "sensors", "scenarios", "cameras"]}).status_code == 400
    assert client.put("/api/user/appearance", json={"security_order": [{}, "zones", "areas", "sensors", "scenarios", "cameras"]}).status_code == 400
    ...
    organization = {
        ...
        "cover-group-no-pct:pt": {"visible": True, "categories": ["covers"], "orders": {"covers": 2}},
    }
    ...
    assert client.put("/api/user/appearance", json={"home_widgets": malformed_widgets}).status_code == 400
...
def test_new_home_widgets_use_the_installation_default_and_are_present(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("EFACE_BACKGROUNDS", str(tmp_path / "backgrounds"))
    client = TestClient(create_app())
    widgets = client.get("/api/user/appearance").json()["home_widgets"]
    assert [(item["id"], item["visible"]) for item in widgets[-6:]] == [
        ("lights_now", True), ("routine_pulse", True), ("shopping_list", True), ("agenda", True), ("pet_feeder", True), ("room_pulse", False)]
    widgets[-1]["visible"] = True
    assert client.put("/api/user/appearance", json={"home_widgets": widgets}).status_code == 200
    assert client.get("/api/user/appearance").json()["home_widgets"][-1]["visible"] is True
    home = client.get("/").text
    for widget_id in ("home-room-pulse", "home-lights-now", "home-routine-pulse", "home-shopping-list", "home-pet-feeder"):
        assert f'id="{widget_id}"' in home
    assert 'data-view="shopping"' in home
    assert 'id="shopping-nav-count"' in home
    assert 'id="agenda-nav-count"' in home
    script = client.get("/assets/app.js").text
    assert "homeShoppingRows" in script
    assert 'data-shopping-section="completed"' in script
...
def test_dynamic_home_settings_are_isolated_by_device(monkeypatch, tmp_path) -> None:
    ...
    assert load_home_widgets("admin") == device_widgets
...
def test_x4_shell_and_brand_assets_are_served() -> None:
    ...
    assert 'id="rooms-toggle"' in page.text
    assert 'Tutti i dispositivi ›' in page.text
    assert 'aria-label="Filtri dispositivi"' in page.text
    assert '<b>Accesi</b>' not in page.text
    assert 'id="light-on-filter"' in page.text
    ...
    assert "function applyHomeWidgetLayout()" in app_js
    assert "dataset.homeWidget" in app_js
    assert "api/home/camera-event" in app_js
    assert "api/home/doorbird/motionsensor" in app_js
    ...
    assert "app.css?v=2.21.266" in client.get("/").text
    assert "home-live-media.css?v=2.21.266" in client.get("/").text
    assert "alarm-state.css?v=2.21.266" in client.get("/").text
    assert "state-glow.css?v=2.21.266" in client.get("/").text
    ...
    assert "const wasMuted=button.dataset.muted==='true'" in app_js
    assert "data-home-room-power" in app_js
...
def test_media_master_volume_contract_is_relative_and_room_label_is_not_duplicated() -> None:
    ...
    assert "const mediaSessionMasterKey = 'eface-media-session-masters-v1'" in script
    assert "rememberMediaSessionMaster(group, activeMediaPlayer.registry_id)" in script
    assert "const owner = mediaSessionMaster(group, starter)" in script
    assert "openDevices(room, devices.length ? devices : [master]" in script
    assert "const linkedWiimRoute = player.transport_provider === 'wiim'" in script
    assert "setTimeout(refresh, 1500)" in script
    assert "if (refreshRunning) { refreshQueued = true; return }" in script
    assert "if (refreshQueued) { refreshQueued = false; queueMicrotask(refresh) }" in script
    assert "function scheduleStateConfirmation()" in script
    assert "function renderOpenStatePanels()" in script
...
def test_buspro_cover_groups_are_devices_not_rooms() -> None:
    ...
    single = next(item for item in normalized["devices"] if item["name"] == "Tapparella")
    assert (single["kind"], single["state_key"]) == ("cover", "1.101.1")
    no_pct = next(item for item in normalized["devices"] if item["id"] == "cover-group-no-pct:ufficio")
    assert (group["kind"], group["category"], group["state"], group["position"]) == (
        "cover", "cover_group", "OPEN", 72)
    assert (no_pct["kind"], no_pct["category"], no_pct["state"], no_pct["position"]) == (
        "cover", "cover_group_no_pct", "OPEN", None)
    assert normalized["counts"]["covers"] == 3
    assert [room["name"] for room in normalized["rooms"]] == ["Ufficio"]


@pytest.mark.asyncio
async def test_buspro_cover_position_requires_capability_and_sends_percent(monkeypatch) -> None:
    import httpx
    from app.config import ProviderConfig
    from app.connectors import buspro as buspro_module
    devices = [{"type": "cover", "name": "Finestra", "subnet_id": 1, "device_id": 104,
                "channel": 2, "use_position": True},
               {"type": "cover", "name": "Gruppo no %", "subnet_id": 1, "device_id": 104,
                "channel": 3, "use_position": False}]
    normalized = normalize_snapshot({"devices": devices})
    assert normalized["devices"][0]["position_supported"] is True
    assert normalized["devices"][1]["position_supported"] is False
    calls = []
    def handler(request):
    ...
    await connector.command("0", "set_position", 8)
    with pytest.raises(ValueError):
        await connector.command("1", "set_position", 8)
    assert calls == [("/api/control/cover/1/104/2", {"command": "SET_POSITION", "position": 8})]
...
async def test_buspro_cover_group_commands_check_live_catalog(monkeypatch) -> None:
    ...
    await connector.command("cover-group-no-pct:ufficio", "stop")
    with pytest.raises(ValueError):
...
def test_device_commands_are_present_in_frontend() -> None:
    ...
    assert "coverStateIcon(device)" in script
    assert "'mdi:window-shutter-open'" in script
    ...
    assert "device?.confirm_action" in script
    assert "window.confirm(`Confermi di voler ${operation}" in script
    assert "data-select-option" in script
    assert "data-home-feeder-run" in script
    assert "data-select-execute" in script
    assert "['switch','select','button']" in script


def test_open_cover_uses_cyan_without_inheriting_lock_red() -> None:
    stylesheet = TestClient(create_app()).get("/assets/state-glow.css").text
    assert ".device-cover-open .device-glyph{color:#61d8f2}" in stylesheet
    assert ".device-lock-open .device-glyph{color:#ff526d}" in stylesheet
    assert ".device-lock-open .device-glyph,.page-device-list article.device-cover-open" not in stylesheet
