...
def test_health() -> None:
    ...
    assert response.json()["version"] == "2.21.168"
...
def test_intercom_is_in_sidebar_with_embedded_view() -> None:
    ...
    assert "const currentVersion = '2.21.168'" in client_script
...
def test_intercom_dashboard_stores_only_local_settings(monkeypatch, tmp_path) -> None:
    ...
    assert "tools-dashboard.js?v=2.21.168" in page
    assert "tools.js?v=2.21.168" in page
    ...
    assert "app.js?v=2.21.168" in home
...
def test_user_appearance_persists_room_order_and_glow(monkeypatch, tmp_path) -> None:
    ...
    cameras = [{"id": "camera_ingresso", "name": "Ingresso", "url": "https://camera.local/live"}, {"id": "camera_giardino", "name": "Giardino", "url": "camera.giardino"}]
