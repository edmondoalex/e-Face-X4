...
def test_health() -> None:
    ...
    assert response.json()["version"] == "2.21.189"
...
def test_intercom_is_in_sidebar_with_embedded_view() -> None:
    ...
    assert "const currentVersion = '2.21.189'" in client_script
...
def test_intercom_dashboard_stores_only_local_settings(monkeypatch, tmp_path) -> None:
    ...
    assert "tools.js?v=2.21.189" in page
    assert "organization-tools.js?v=2.21.189" in page
    ...
    assert "app.js?v=2.21.189" in home
...
def test_x4_shell_and_brand_assets_are_served() -> None:
    ...
    assert "app.css?v=2.21.189" in client.get("/").text
    assert "home-live-media.css?v=2.21.189" in client.get("/").text
    assert "alarm-state.css?v=2.21.189" in client.get("/").text
    assert ".home-event-dialog figure img{display:block;width:auto;height:auto;max-width:100%;max-height:100%" in css
    assert ".home-event-widget img{object-fit:contain" in css
