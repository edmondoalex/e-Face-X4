...
def test_health() -> None:
    ...
    assert response.json()["version"] == "2.21.249"
...
def test_intercom_is_in_sidebar_with_embedded_view() -> None:
    ...
    assert "const currentVersion = '2.21.249'" in client_script
...
def test_intercom_dashboard_stores_only_local_settings(monkeypatch, tmp_path) -> None:
    ...
    assert "tools-dashboard.js?v=2.21.249" in page
    assert "tools-dashboard.css?v=2.21.249" in page
    assert "tools.js?v=2.21.249" in page
    assert "organization-tools.js?v=2.21.249" in page
    ...
    assert "app.js?v=2.21.249" in home
...
def test_tools_page_starts_with_selected_background_and_card_theme(monkeypatch, tmp_path) -> None:
    ...
    assert 'ui-theme-contract.css?v=2.21.249' in home
    assert 'app.js?v=2.21.249' in home
...
def test_user_appearance_persists_room_order_and_glow(monkeypatch, tmp_path) -> None:
    ...
    assert [item["id"] for item in appearance["home_widgets"]] == ["overview", "weather", "camera_event", "doorbell", "motion", "states", "rooms", "live", "room_pulse", "lights_now", "routine_pulse", "shopping_list", "agenda"]
    assert all(not item["visible"] for item in appearance["home_widgets"][-5:])
...
def test_new_home_widgets_are_opt_in_and_present_in_the_home(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("EFACE_BACKGROUNDS", str(tmp_path / "backgrounds"))
    client = TestClient(create_app())
    widgets = client.get("/api/user/appearance").json()["home_widgets"]
    assert [(item["id"], item["visible"]) for item in widgets[-5:]] == [
        ("room_pulse", False), ("lights_now", False), ("routine_pulse", False), ("shopping_list", False), ("agenda", False)]
...
def test_x4_shell_and_brand_assets_are_served() -> None:
    ...
    assert "app.css?v=2.21.249" in client.get("/").text
    assert "home-live-media.css?v=2.21.249" in client.get("/").text
    assert "alarm-state.css?v=2.21.249" in client.get("/").text
    assert "state-glow.css?v=2.21.249" in client.get("/").text
