...
def test_health() -> None:
    ...
    assert response.json()["version"] == "2.21.231"
...
def test_intercom_is_in_sidebar_with_embedded_view() -> None:
    ...
    assert "const currentVersion = '2.21.231'" in client_script
...
def test_intercom_dashboard_stores_only_local_settings(monkeypatch, tmp_path) -> None:
    ...
    assert "tools-dashboard.js?v=2.21.231" in page
    assert "tools-dashboard.css?v=2.21.231" in page
    assert "tools.js?v=2.21.231" in page
    assert "organization-tools.js?v=2.21.231" in page
    ...
    assert "app.js?v=2.21.231" in home
...
def test_tools_page_starts_with_selected_background_and_card_theme(monkeypatch, tmp_path) -> None:
    ...
    assert 'ui-theme-contract.css?v=2.21.231' in home
    assert 'app.js?v=2.21.231' in home
...
def test_x4_shell_and_brand_assets_are_served() -> None:
    ...
    assert "app.css?v=2.21.231" in client.get("/").text
    assert "home-live-media.css?v=2.21.231" in client.get("/").text
    assert "alarm-state.css?v=2.21.231" in client.get("/").text
    assert "state-glow.css?v=2.21.231" in client.get("/").text
